...
function renderRequestsTab() {
    ...
    // Filter by team if selector is present
    const teamFilterEl = document.getElementById('requestsTeamFilter');
    const selectedTeam = teamFilterEl ? teamFilterEl.value : 'All';
    if (selectedTeam !== 'All') {
        if (selectedTeam === 'Unassigned') {
            pendingRequests = pendingRequests.filter(t => {
                const team = (t.assignedTeam || '').trim();
                return !team || team === 'Unassigned' || team === 'N/A';
            });
        } else {
            pendingRequests = pendingRequests.filter(t => {
                const teams = (t.assignedTeam || '').split(',').map(s => s.trim());
    ...
    container.innerHTML = pendingRequests.map(task => {
        ...
        return `
        <div class="task-card" style="border-left:3.5px solid var(--team-avionics); cursor: default; padding: 1.25rem; display: flex; flex-direction: column; justify-content: space-between; gap: 0.75rem;" id="request-card-${task.id}">
            <div>
                <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:0.5rem;margin-bottom:0.5rem;">
                    <div style="display:flex;flex-direction:column;gap:0.25rem;min-width:0;">
                        <span style="font-size:0.95rem;font-weight:700;color:var(--text-primary);">${topic}</span>
                        <span style="font-size:0.8rem;font-weight:600;color:var(--text-secondary);">${task.aircraftReg} (${task.aircraftType || 'N/A'}) &middot; ATA ${task.ataChapter || 'N/A'}</span>
                    </div>
                    <span style="font-size:0.7rem;font-weight:700;color:${prioColor};background:${prioColor}15;border:1px solid ${prioColor}30;padding:0.2rem 0.5rem;border-radius:4px;white-space:nowrap;text-transform:uppercase;">${task.priorityLevel}</span>
                </div>
                
                <div style="font-size:0.82rem;color:var(--text-secondary);margin:0.5rem 0;line-height:1.4;background:var(--bg-card);padding:0.6rem;border-radius:6px;border:1px solid var(--border-color);white-space:pre-wrap;">${task.taskDescription}</div>
                
                <div style="display:flex;flex-direction:column;gap:0.35rem;font-size:0.78rem;color:var(--text-muted);border-top:1px solid var(--border-color);padding-top:0.65rem;margin-top:0.65rem;">
                    <div style="display:flex;align-items:center;gap:6px;">
                        <strong style="color:var(--text-secondary);">Requested By:</strong> ${task.requestor}
                    </div>
                    <div style="display:flex;align-items:center;gap:6px;">
                        <strong style="color:var(--text-secondary);">Contact:</strong> ${task.requestorContact || 'N/A'}
                    </div>
                    <div style="display:flex;align-items:center;gap:6px;">
                        <strong style="color:var(--text-secondary);">Created:</strong> ${task.createdDate || 'N/A'}
                    </div>
                    ${attachCount > 0 ? `
                    <div style="display:flex;align-items:center;gap:6px;color:var(--team-avionics);">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:12px;height:12px;"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"></path></svg>
                        ${attachCount} Attachment(s)
                    </div>` : ''}
                </div>
            </div>
            
            <div style="display:flex;gap:0.5rem;margin-top:0.5rem;border-top:1px solid var(--border-color);padding-top:0.75rem;">
                <button class="btn btn-primary" onclick="approveRequestDirect('${task.id}')" style="flex:1;font-size:0.78rem;padding:0.4rem 0.5rem;height:auto;min-height:auto;gap:4px;background:var(--team-avionics);color:#fff;" title="Move task to Open on Kanban board">
                ...
                <button class="btn btn-secondary" onclick="editRequestDirect('${task.id}')" style="font-size:0.78rem;padding:0.4rem 0.5rem;height:auto;min-height:auto;gap:4px;" title="Edit details or assign team">
                ...
                <button class="btn btn-secondary" onclick="rejectRequestDirect('${task.id}')" style="font-size:0.78rem;padding:0.4rem 0.5rem;height:auto;min-height:auto;color:var(--priority-aog);border-color:rgba(220,38,38,0.2);" title="Reject and delete request">
