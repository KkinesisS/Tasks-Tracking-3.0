...
function renderApp() {
    ...
    // Refresh calendar tab if it's currently active/visible
...
function renderWeeklySummaryTab() {
    ...
    if (sortedTasks.length === 0) {
        ...
        sortedTasks.forEach(t => {
            ...
            row.className = `task-row prio-${prioClass} status-${statusClass} ${teamClass}${hasLog ? ' has-log' : ''}`;
            ...
            if (updates.length > 0) {
                ...
                logRow.innerHTML = `
                    <td colspan="8" style="padding:0;border-top:none;">
                        <div style="padding:1rem 1.5rem 1.25rem 1.5rem;background:var(--bg-card);border-bottom:1px solid var(--border-color);">
...
function renderCalendarCell(day, month, year, dateStr, isOtherMonth) {
    ...
    const cellTasks = tasks.filter(t => {
        if (!t.rtsDate) return false;
        const taskDate = t.rtsDate.split('T')[0];
    ...
    visibleTasks.forEach(task => {
        ...
        tasksHtml += `
            <div class="calendar-task-badge ${teamClass}" 
                 ...
                 title="${task.aircraftReg} - ${task.taskDescription} (${task.currentStatus})">
                ${statusText}${priorityText}${task.aircraftReg || 'N/A'}
            </div>
        `;
    });
