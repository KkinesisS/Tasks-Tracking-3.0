...
function startApp() {
    ...
    const storedUrl = localStorage.getItem('mro_power_automate_url');
    if (storedUrl === null || 
...
function setupEventListeners() {
    ...
    document.getElementById('addUpdateBtn').addEventListener('click', () => {
        ...
        if (fileInput && fileInput.files.length > 0) {
            ...
            const powerAutomateUrl = localStorage.getItem('mro_power_automate_url');
...
function exportToExcel() {
    ...
    // File name: MRO_Weekly_Report_YYYY-MM-DD.csv
    const dateStamp = new Date().toISOString().split('T')[0];
    link.href = url;
    link.download = `MRO_Weekly_Report_${dateStamp}.csv`;
...
function renderWeeklySummaryTab() {
    ...
    teamsToRender.forEach(teamName => {
        ...
        if (completedTasks.length > 0) {
            const details = completedTasks.map(t => `${t.aircraftReg} (${t.aircraftType}): ${truncateSummaryString(t.taskDescription, 60)}`).join(', ');
            sentences.push(`Successfully completed ${completedTasks.length} task(s): ${details}.`);
        }
        ...
        if (activeTasks.length > 0) {
            ...
            if (aogActive.length > 0) {
                const details = aogActive.map(t => `${t.aircraftReg} (${t.aircraftType}): ${truncateSummaryString(t.taskDescription, 60)}`).join(', ');
                sentences.push(`<span style="color: var(--priority-aog); font-weight: 600;">CRITICAL: Managing ${aogActive.length} active AOG defect(s) on ${details}.</span>`);
            }
            if (normalActive.length > 0) {
                const details = normalActive.map(t => `${t.aircraftReg} (${t.aircraftType})`).join(', ');
                sentences.push(`Currently progressing ${normalActive.length} active task(s) on ${details} (Status: ${normalActive.map(t => t.currentStatus).join(', ')}).`);
            }
        }
        
        narrativeHtml += `<p style="margin: 0; padding-left: 1.25rem;">${sentences.join(' ')}</p></div>`;
    ...
    if (sortedTasks.length === 0) {
        ...
        sortedTasks.forEach(t => {
            ...
            let updatesHtml = '<span style="color: var(--text-muted); font-style: italic; font-size: 0.75rem;">No updates.</span>';
            if (updates.length > 0) {
                const sortedUpdates = [...updates].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
                updatesHtml = `<div style="display: flex; flex-direction: column; gap: 4px; font-size: 0.78rem; line-height: 1.35; max-width: 250px; text-align: left;">` + 
                    sortedUpdates.map(u => {
                        let dateStr = '';
                        try {
                        ...
                        return `<div><strong>[${dateStr}] ${u.author}:</strong> ${escapeHTMLApp(u.text)}</div>`;
                    }).join('') + `</div>`;
            }
...
function uploadFyiFile(fileInput) {
    ...
    reader.onload = function(e) {
        handleCloudOrLocalUpload(file, e.target.result, (url) => {
            ...
            document.getElementById('fyiAttachmentName').value = file.name;
            alert(`File "${file.name}" cached locally!`);
        }, fyiTeam || 'FYI', dateStr, 'N-A', fyiTitle);
    };
    reader.readAsDataURL(file);
}
